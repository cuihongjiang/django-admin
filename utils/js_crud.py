...
from manageSys.settings import BASE_DIR, STATIC_URL
from ninja import Schema
from openpyxl import load_workbook

from .mixins import DataPermissionMixin
from .js_ninja import JsFilters
...
# 增加类型注解和文档字符串
def create(request: HttpRequest, data: dict, model: Model) -> Model:
    """标准化创建方法\n\n    Args:\n        request: 请求对象\n        data: 已验证的请求数据\n        model: 目标模型类\n    """
    user_info = {
        'id': request.user.id,
    ...
    data.update({
        ...
        'modifier': user_info['name'],
        'belong_dept_id': user_info['dept']
    })
...
def retrieve(request, model, filters: JsFilters = JsFilters()):
    """
    ...
    - filters: JsFilters类的实例，包含过滤条件。默认为JsFilters()，即无条件过滤。

    返回值:
    - query_set: 一个Django QuerySet对象，包含根据过滤条件检索到的模型实例。
    """
    # 根据请求和过滤条件应用数据权限控制
    filters = DataPermissionMixin(request, filters)
...
def import_data(request, model, scheme, data, import_fields):
    ...
    return ResponseUtils.success(msg='导入成功')  # 返回成功消息
