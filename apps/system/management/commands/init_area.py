...
def main():
    with open(os.path.join(BASE_DIR, 'utils', 'init', 'pca-code.json'), 'r', encoding="utf-8") as load_f:
        ...
        if hasattr(connection, 'tenant') and connection.tenant.schema_name:
            ...
            for tenant in get_tenant_model().objects.exclude(schema_name='public'):
                with tenant_context(tenant):
                    self.stdout.write(f"租户[{connection.tenant.schema_name}]初始化数据开始...")
                    main()
                    self.stdout.write(f"租户[{connection.tenant.schema_name}]初始化数据完成！")
        else:
            main()
        self.stdout.write(self.style.SUCCESS("省份数据初始化数据完成！"))
